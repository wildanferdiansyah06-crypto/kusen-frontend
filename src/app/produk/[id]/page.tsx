...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const toggleWishlist = () => {
    ...
    const wishlist = JSON.parse(localStorage.getItem('wishlist') || '[]');
    if (isWishlisted) {
      const newWishlist = wishlist.filter((id: number) => id !== parseInt(params.id));
  ...
  const getImageUrl = () => {
    ...
    return categoryImages[kusen?.kategori || ''] || 'https://images.pexels.com/photos/1571460/pexels-photo-1571460.jpeg?auto=compress&cs=tinysrgb&w=800';
  };
                  ...
                  {['description', 'specs', 'care'].map((tab) => (
                    <button
                      ...
                      className={`pb-4 font-medium transition-colors ${
                        activeTab === tab
                          ? 'text-[var(--color-teak)] border-b-2 border-[var(--color-teak)]'
