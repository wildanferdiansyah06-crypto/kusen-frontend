...
function SearchPageContent() {
  ...
  const [filteredKusen, setFilteredKusen] = useState([]);
    ...
    fetchKusenList().then((data) => {
      if (query) {
        const filtered = data.filter(
          (k) => k.nama.toLowerCase().includes(query.toLowerCase()) || k.deskripsi.toLowerCase().includes(query.toLowerCase()) || k.kategori.toLowerCase().includes(query.toLowerCase()) || k.jenisKayu.toLowerCase().includes(query.toLowerCase())
        );
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100">
    ...
    /* Search Header */
        ...
        {loading ? <div className="text-center py-20">
            ...
            {filteredKusen.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredKusen.map((kusen) => <div key={kusen.id} className="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 border border-gray-100">
                    <div className="relative overflow-hidden">
                      {kusen.gambarUrl ? <Image
    src={kusen.gambarUrl}
    alt={kusen.nama}
    width={400}
    height={200}
  ...
  /> : <div className="w-full h-56 bg-gradient-to-br from-slate-200 to-gray-300 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                      ...
                      {kusen.terjual < 10 && <span className="absolute top-3 left-3 bg-green-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-md">Baru</span>}
                      {kusen.terjual >= 50 && <span className="absolute top-3 left-3 bg-slate-700 text-white px-3 py-1 rounded-full text-xs font-bold shadow-md">Best Seller</span>}
                      {!kusen.tersedia && <span className="absolute top-3 right-3 bg-red-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-md">Habis</span>}
  ...
  >
                          {wishlist.includes(kusen.id) ? "\u2764\uFE0F" : "\u{1F90D}"}
                        </button>
                        <button
  ...
  >
              ...
              </div> : <div className="bg-white rounded-3xl shadow-2xl p-12 text-center border border-gray-100">
...
function SearchPage() {
  return <Suspense fallback={<div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-100 flex items-center justify-center"><div className="text-2xl font-bold text-orange-600">Loading...</div></div>}>
      <SearchPageContent />
    </Suspense>;
}
export {
