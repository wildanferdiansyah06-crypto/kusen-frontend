...
function MaterialShowcase() {
  ...
  const renderStars = (count) => {
    return [...Array(5)].map((_, i) => <span
      key={i}
      className={`text-sm ${i < count ? "text-[var(--color-gold)]" : "text-[var(--color-mist)]"}`}
    >
        ★
      </span>);
  };
  return <section className="section bg-[var(--color-parchment)]">
    ...
    className={`w-full text-left p-4 rounded-lg transition-all duration-300 ${activeMaterial.id === material.id ? "bg-[var(--color-walnut)] text-white border-l-4 border-[var(--color-gold)]" : "bg-[var(--color-linen)] text-[var(--color-stone)] hover:bg-[var(--color-cream)]"}`}
  >
