...
export function ProductCard({ kusen }: ProductCardProps) {
  ...
  const toggleWishlist = (e: React.MouseEvent) => {
    ...
    if (isWishlisted) {
      const newWishlist = wishlist.filter((id: number) => id !== kusen.id);
  ...
  const getImageUrl = () => {
    ...
    // Use Pexels images based on category
  ...
  return (
    <Link href={`/produk/${kusen.id}`} className="group block">
      <div className="product-card overflow-hidden">
        ...
        <div className="relative h-[280px] overflow-hidden bg-[var(--color-linen)]">
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            ...
            {kusen.stok < 5 && kusen.stok > 0 && (
              <KusenBadge variant="warning">Stok Terbatas</KusenBadge>
            )}
          </div>
